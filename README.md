...
## 4b. Khoá đồng thời (concurrency lock) — mới thêm, khắc phục đúng giới hạn ở mục 4

Giờ 2 người cùng sửa 1 hợp đồng/khách hàng cùng lúc → ai lưu sau sẽ nhận cảnh báo "dữ liệu đã bị người khác thay đổi" và bị từ chối lưu đè, thay vì âm thầm ghi đè mất thông tin người kia. Cần chạy `migration_concurrency_lock.sql`.

Cơ chế: mỗi bản ghi có cột `updated_at` tự cập nhật qua trigger mỗi lần sửa; app "nhớ" giá trị này lúc mở form, kiểm tra lại lúc lưu.
...
7. `migration_concurrency_lock.sql` — khoá đồng thời (cảnh báo khi 2 người cùng sửa 1 bản ghi)
